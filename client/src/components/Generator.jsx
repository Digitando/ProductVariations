...
function Generator({ onSessionComplete, onViewImage, token, coins = 0, onCoinsChange, onRequestTopUp }) {
  ...
  const filteredPrompts = useMemo(() => {
    ...
    return allPrompts.filter((prompt) => {
      const promptSubcategoryIds = Array.isArray(prompt.subcategoryIds) ? prompt.subcategoryIds : []
      if (promptSubcategoryIds.length === 0) {
        return true
      }
      return promptSubcategoryIds.includes(selectedSubcategoryId)
    })
  }, [activeCategory, hasSubcategoryOptions, selectedSubcategoryId])

  const filteredPromptIds = useMemo(() => filteredPrompts.map((prompt) => prompt.id), [filteredPrompts])
  const filteredPromptIdSet = useMemo(() => new Set(filteredPromptIds), [filteredPromptIds])
  ...
  const filteredPromptGroups = useMemo(() => {
    ...
    const allowedIds = new Set(filteredPromptIds)

    return (activeCategory.groups || [])
      .map((group) => {
        const prompts = (group.prompts || []).filter((prompt) => allowedIds.has(prompt.id))
        return prompts.length > 0 ? { id: group.id, label: group.label, prompts } : null
      })
      .filter(Boolean)
  }, [activeCategory, filteredPromptIds])
  ...
  useEffect(() => {
    ...
    setSelectedPromptIds((prev) => {
      ...
      const allowed = new Set(filteredPromptIds)
      const next = prev.filter((id) => allowed.has(id))
      return next.length === prev.length ? prev : next
    })
  }, [filteredPromptIds, selectedPromptIds.length])

  const promptContextLabel = useMemo(() => {
    const parts = []
    if (activeCategoryLabel) {
      parts.push(activeCategoryLabel)
    }
    if (activeSubcategoryLabel) {
      parts.push(activeSubcategoryLabel)
    }
    return parts.join(' · ')
  ...
  const focusHasTemplates = Boolean(activeCategory) && filteredPromptCount > 0
  ...
  const togglePrompt = (promptId) => {
    if (!filteredPromptIdSet.has(promptId)) {
      return
    }
  ...
  const handleSubmit = async (evt) => {
    ...
    if (!activeCategory || filteredPromptCount === 0) {
              ...
              <div className="styles">
                <div className="styles__header">
                  <div>
                    ...
                    {promptContextLabel && <p className="styles__context">{promptContextLabel}</p>}
                  </div>
                ...
                {activeCategory && hasSubcategoryOptions && !selectedSubcategoryId && (
                  <p className="styles__hint">Select a subcategory to load prompt templates.</p>
                )}
                {activeCategory && (!hasSubcategoryOptions || selectedSubcategoryId) && filteredPromptCount === 0 && (
                  <p className="styles__hint">Prompt templates for this selection are coming soon.</p>
                )}
                {activeCategory && filteredPromptCount > 0 && (
                  <>
